...
function validateContactInfo(response) {
    ...
    // Комментарий
    respObj.comments != undefined ? successContactData.comments = `\nКомментарий: ${clearComments(respObj.comments)}` : successContactData.comments = ''
    // Должность
...
function validateCompanyInfo(objData) {
    ...
    if (respObj.comments != undefined) {
        successData.successDataComments = `\nКомментарий: ${clearComments(respObj.comments)}`;
...
//* Функция очищающая комментарий от html тегов и переносов строк
function clearComments(comment) {
    let htmlToPlain = convertToPlain(comment.replace(/\r?\n/g, "").replace(/&nbsp;/g, " "));
    return htmlToPlain;
}
